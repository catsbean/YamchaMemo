  ...
  return (
    <div className="flex flex-col gap-6 text-sm leading-relaxed text-neutral-700">
      ...
      <Topic title="분류 — 왼쪽 메뉴">
        <Dl
          items={[
            ["🏠 홈", "오늘 할 일·읽는 중인 책·최근 노트를 한 화면에 모아 봅니다. 할 일은 여기서 바로 체크합니다."],
            [
              "☑ 할 일",
              "모든 글에 흩어진 할 일을 적힌 곳별로 모아 봅니다. 눌러서 완료 표시하거나 그 글로 갑니다. 새 할 일은 오늘 일지에 담깁니다(없으면 만듭니다). [완료한 할 일 보기]로 끝낸 것도 볼 수 있고, 설정 > 일반에서 이 탭을 끌 수 있습니다.",
            ],
        ...
        <p>
          ...
          입력칸과 새 노트 서식도 함께 정합니다. 입력칸의 <B>[목록]</B>을 켜면 그
          분류의 목록에서 제목 옆에 그 칸의 값이 함께 보입니다 — 나중에 설정 &gt;
          기록 &gt; 사용자 추가 분류에서 바꿀 수 있습니다.
        </p>
        <p>
      ...
      <Topic title="노트끼리 잇기">
        ...
        <p>
          ...
          걸립니다. 같은 이름의 글에 가려 쓰이지 않는 별칭은 입력칸이 그 자리에서
          알려 주고, <B>나중에 같은 이름의 글이 생겨 별칭이 죽은 경우</B>나 두 글이
          같은 별칭을 나눠 가진 경우는 <B>[⚠️ 점검]</B>에 모입니다.
        </p>
        <p>
      ...
      <Topic title="잃어버리지 않게 하는 장치들">
        <Dl
          items={[
            ...
            ["점검", "규격에서 벗어나 목록에 안 보이는 파일이나, 적어 뒀는데 작동하지 않는 별칭이 있으면 왼쪽 아래에 [⚠️ 점검]이 뜹니다. 대부분 눌러서 고칠 수 있습니다."],
          ]}
        />
      </Topic>
      ...
      <Topic title="이럴 땐">
        <Dl
          items={[
            ["차례를 바꾸고 싶다", "목록 위 [정렬]에서 날짜·제목·분류 칸을 고릅니다. 켜져 있는 것을 한 번 더 누르면 차례가 뒤집힙니다(↑↓). 책장은 작가·분야·상태·평점으로도 세울 수 있고, 목록 보기에서는 표 머리글을 눌러도 됩니다. 고른 차례는 분류마다 따로 기억합니다."],
