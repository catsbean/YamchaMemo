...
27. **퍼지 검증(`fuzzy_score`)은 tantivy에 STORED로 넣은 값만 본다.** 색인만 하고 저장하지
    ...
    경계를 넘어 없는 말을 만든다).
28. **링크를 일괄 수정해도 되는지는 "그 링크가 한 파일을 못 박았는가"로 갈린다.**
    `[[Free/메모]]`는 정확히 한 파일이었으므로 옮길 때 `[[회의록/메모]]`로 고치는 것이
    언제나 옳다(`replace_path_links`). 반대로 `[[메모]]`는 옮기고 나면 같은 이름의 글이
    둘이 될 수 있어, 원래 있던 글을 뜻했는지 옮겨 온 글을 뜻했는지 글자만으로는 알 수
    없다 — 그건 고치지 않고 알린다. 한 규칙으로 뭉뚱그리면 둘 중 하나가 반드시 틀린다.
29. **점검(`audit.rs`)에는 성격이 다른 두 갈래가 있다.** 파일 규격 문제는 파일 하나만 보면
    되므로 `walk`가 파일당 하나씩 찾는다. 별칭 문제는 **vault 전체의 이름 관계**라야
    보이므로 `alias_issues`가 따로 돈다(그래서 한 파일이 두 항목을 낼 수 있다).
    거기서 파일을 다시 읽지 말 것 — `list_notes()`의 요약은 (수정시각, 크기)로 캐시되어
    있어서, 그걸 쓰면 점검을 눌러도 vault를 두 번 읽지 않는다.
30. **점검의 `fix`는 목록에 적힌 값이 아니라 그 순간 다시 계산한 값으로 고쳐야 한다.**
    점검 화면을 띄워 둔 사이에 상황이 바뀔 수 있다 — 가리던 글이 지워졌는데 화면에
    적힌 대로 별칭을 빼면 멀쩡한 별칭을 잃는다(`live_aliases`).
31. **`.md`를 뗄 때 `trim_end_matches`를 쓰지 말 것 — 붙은 만큼 반복해서 뗀다.**
    제목이 `메모.md`인 글은 파일이 `Free/메모.md.md`가 되는데, 여기서 `메모`가 나온다.
    같은 자리를 계산하는 `Path::file_stem()`과 프론트의 `stemOf`는 `메모.md`를 준다 —
    §22와 같은 종류의 어긋남이고, 손으로 적은 쪽만 틀린다. 점검에서는 남의 멀쩡한 별칭을
    죽었다고 보고해 **[고치기]가 지웠고**, `replace_path_links`에서는 옆 글(`Free/메모.md`)의
    링크를 고치면서 정작 옮긴 글의 링크는 놓쳤다. `strip_suffix(".md").unwrap_or(name)`.
32. **같은 이름 문제를 두 갈래로 보고할 때, 앞 갈래에 걸린 것은 뒤 갈래에서 빼야 한다.**
    해석기는 제목 겹에서 끊으므로(§22), 별칭이 이미 가려졌으면 별칭 겹은 아예 돌지 않는다.
    그런데 겹침 검사가 그걸 모르면 "누를 때마다 어느 글인지 묻게 됩니다"를 함께 띄운다 —
    묻는 일은 일어나지 않고, 겹침은 고칠 수도 없어서 어느 글을 정하든 아무것도 안 달라진다.
    고칠 데는 가려진 별칭 하나뿐이다. 보고를 갈라 놓았으면 **갈래끼리 겹치지 않는지**도 본다.
...
**제목을 바꾸는 사이에 밟은 것 (0.6.2 이후)**

36. **경로를 바꾸는 커맨드(`rename_note`·`move_note`)는 새 경로에 파일을 다시 쓴다** —
    title·type을 넣으려고. 그래서 화면이 든 지문(`stamp`)은 그 순간 낡는다. 예전
    `renameCurrent`는 왕복 뒤 `openNote`로 다시 읽어 지문을 맞췄는데, 그 사이에 친
    글자가 있으면 **먼저 저장하려다 충돌("외부에서 수정됨")**이 나고, 다시 읽으면서
    친 글자는 디스크 내용에 덮였다. 지금은 왕복 직후 새 파일을 읽어 **지문과 백엔드가
    고친 필드만 얹고**(`relocateCurrent`의 `carry`), 본문·frontmatter는 dirty면 화면 것을
    지킨다. 마무리도 다시 읽지 않는다(`settleRelocated`).
37. **편집기 `key`를 `rel_path`로 두면 제목을 바꿀 때마다 CodeMirror가 새로 만들어진다.**
    커서·조합 중인 한글·되돌리기 기록이 날아가고, 그 순간 치던 글자는 사라진다. 노트를
    갈아탄 횟수(`openSeq`)를 key로 쓴다 — `openNote`가 다른 rel로 갈 때만 오른다.
38. **왕복이 길면(링크 치환·`reindex_all`) 감시가 옛 경로의 "사라짐"을 외부 수정으로 알린다.**
    화면은 아직 옛 경로라 `changed.includes(cur.rel_path)`에 걸려 경고가 뜨거나, 없는 파일을
    다시 읽으려다 오류가 났다. 옮기는 동안의 옛 경로(`relocatingFrom`)는 거른다.
39. **제목칸에서 Enter를 치면 칸이 사라지며 blur가 한 번 더 온다(Chromium).** 두 번째 호출이
    같은 이름으로 또 바꾸려다 "없는 파일" 오류를 냈다. `titleCommitted` ref로 한 번만 받는다.
